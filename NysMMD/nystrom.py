...
## tensorlfow version
def nystrom_features_tf(X, nys_inds, sigma):
    Z = tf.gather(X, nys_inds)                    # (k, d)
    Knm = RBFkernel_tf(X, Z, sigma)                 # (n, k)
    Km = RBFkernel_tf(Z, Z, sigma)                  # (k, k)
    ...
    return tf.matmul(Knm, Km_inv_sqrt)
